...
Saves frames as PNG images and metadata as JSONL format.
"""
...
DEFAULT_RECORDINGS_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "recordings")
...
class Recorder:
    ...
    def record_frame(
        ...
        # Save frame image
        frame_name = f"frame_{save_idx:05d}.png"
        frame_path = self.images_dir / frame_name

        # Save PIL Image as PNG (lossless, avoids domain shift vs inference)
        observation.frame.save(str(frame_path), "PNG")

...
class FPS:
    ...
    def elapsed_time(self):
        ...
        total_time = delay + elapsed_time
        if total_time > 0:
            self.fps = 1.0 / total_time
        else:
            self.fps = 0
    ...
    def cap_fps(self, elapsed_time, cap=0):
        ...
        if cap:
            ...
            # Only sleep if delay is meaningful (> 0.001 seconds)
            if delay > 0.001:
                try:
                    time.sleep(delay)
                except Exception:
                    # If sleep fails, just continue without sleeping
                    pass
        return delay

    def get_fps(self):
        times_list = self.times_taken.get_list()
        if not times_list:
            return 0
        fps = sum(times_list) / len(times_list)
        return fps
